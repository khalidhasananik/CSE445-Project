...
# Define function to fetch weather data for a given district
...
# Define function to save weather data to CSV
def save_to_csv(data, filename="district_weather_2024.csv"):
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        header = ['District', 'Year'] + \
                 [f'MinTemp{month}' for month in range(1, 13)] + \
                 [f'MaxTemp{month}' for month in range(1, 13)] + \
                 [f'Rainfall{month}' for month in range(1, 13)] + \
                 [f'Humidity{month}' for month in range(1, 13)] + \
                 [f'Sunshine{month}' for month in range(1, 13)] + \
                 [f'WindSpeed{month}' for month in range(1, 13)] + \
                 [f'CloudCoverage{month}' for month in range(1, 13)]
        writer.writerow(header)

        for district, coords in data.items():
            lat, lon = coords
            weather_data = fetch_weather_data(lat, lon)

            monthly_data = {'District': district, 'Year': 2024}
            for month in range(1, 13):
                # Assume the API returns hourly data for the entire month; otherwise, we'll need to adjust
                month_data = weather_data.get('hourly', [])

                # Default values if no data available for that month
                min_temp, max_temp, rainfall, humidity, sunshine, wind_speed, cloud_coverage = None, None, None, None, None, None, None

                # Loop through the hourly data and aggregate
                for hour in month_data:
                    date = datetime.utcfromtimestamp(hour['dt'])
                    if date.month == month:
                        min_temp = min(min_temp, hour['temp']) if min_temp is not None else hour['temp']
                        max_temp = max(max_temp, hour['temp']) if max_temp is not None else hour['temp']
                        rainfall = hour.get('rain', {}).get('1h', 0) if rainfall is None else rainfall + hour.get('rain', {}).get('1h', 0)
                        humidity = hour['humidity'] if humidity is None else humidity + hour['humidity']
                        sunshine = hour.get('sunshine', 0) if sunshine is None else sunshine + hour.get('sunshine', 0)
                        wind_speed = hour['wind_speed'] if wind_speed is None else wind_speed + hour['wind_speed']
                        cloud_coverage = hour['clouds'] if cloud_coverage is None else cloud_coverage + hour['clouds']

                # Save aggregated data
                monthly_data[f'MinTemp{month}'] = min_temp
                monthly_data[f'MaxTemp{month}'] = max_temp
                monthly_data[f'Rainfall{month}'] = rainfall
                monthly_data[f'Humidity{month}'] = humidity / len(month_data) if month_data else None
                monthly_data[f'Sunshine{month}'] = sunshine
                monthly_data[f'WindSpeed{month}'] = wind_speed
                monthly_data[f'CloudCoverage{month}'] = cloud_coverage

            # Write row for each district
            writer.writerow(monthly_data.values())

# Fetch data for all districts and save to CSV
save_to_csv(districts)
...
#     avg_wind_speed = day_data['day']['maxwind_kph']  # wind speed in km/h
#     cloud_cover = day_data['day']['avgvis_km']  # Using visibility as a proxy for cloud cover (alternative: cloud)
#     sunshine_hours = day_data['day']['sunshine']  # Direct sunshine data

#     return min_temp, max_temp, avg_wind_speed, cloud_cover, sunshine_hours

# def process_monthly_data(data):
#     """Aggregate daily data into monthly averages and sums."""
#     min_temp_sum, max_temp_sum, wind_speed_sum, cloud_sum, sunshine_sum, count = 0, 0, 0, 0, 0, 0
#     for day in data.get('forecast', {}).get('forecastday', []):
#         min_temp, max_temp, wind_speed, cloud, sunshine = process_daily_data(day)
...
#         year = 2024
#         min_temps, max_temps, wind_speeds, cloud_covs, sunshines = [], [], [], [], []
...
#             if data:
#                 min_temp, max_temp, wind_speed, cloud_cover, sunshine = process_monthly_data(data)
#                 min_temps.append(min_temp)
#                 max_temps.append(max_temp)
#                 wind_speeds.append(wind_speed)
#                 cloud_covs.append(cloud_cover)
...
#         row = [district, year] + min_temps + max_temps + wind_speeds + cloud_covs + sunshines
...
#         [f'WindSpeed_{i}' for i in range(1, 13)] +
#         [f'CloudCover_{i}' for i in range(1, 13)] +
#         [f'Sunshine_{i}' for i in range(1, 13)]
#     )

#     # Create a DataFrame and save it to Excel
#     df = pd.DataFrame(output_data, columns=columns)
#     df.to_excel("BangladeshClimateData2024updated.xlsx", index=False)
#     print("✅ Data extraction complete. Saved to BangladeshClimateData2024.xlsx.")
...
#     df = pd.DataFrame(output_data, columns=columns)
#     df.to_excel("BangladeshClimateDataUpdated.xlsx", index=False)
